...
"""
stand alone read qc script based on jgi-rqc-pipeline/readqc/readqc.py
...
from readqc_constants import RQCReadQcConfig, RQCReadQc, ReadqcStats, RQCReadQcCommands
...
def do_sketch_vs_nt_refseq_silva(fasta, log):
    bbtooldir = os.path.join(os.path.dirname(__file__), os.path.pardir)
    ...
    sendSketchShCmd = os.path.join(bbtooldir, 'sendsketch.sh')   #get_tool_path("sendsketch.sh", "bbtools")
...
def stetch_section_note():
    overview = '''
            Sketch tool uses a technique called MinHash to rapidly compare
            large sequences. The result is similar to BLAST, a list of hits from a
            query sequence to various reference sequences, sorted by similarity
            but the mechanisms are very different. Sketch is designed to be
            extremely fast (potentially thousands of times faster than BLAST) while
            having a very low disk and memory footprint, but gives more approximate
            results and does not produce alignments. The sensitivity is typically
            lower compared to alignment when seeking remote homology, though default
            parameters are adequate to identify the species of a raw PacBio library
            (~85% identity to reference), and detect homology between full bacterial
            genomes with 76% identity. The Sketch tools support both nucleotide and
    ...
    legend = {
        ...
        'No-Hit'    : 'Number of kmers that did not hit any reference sequence.  Though constant for a query, it will be reported differently for different references based on the relative size of the reference and query (if the reference is bigger than the query, it will report all of them).',
    ...
    html = ''
    for name in legend:
        html += html_tag('li', legend[name])
    html ='Overview<br />%s<br />' % html_tag('p', ' '.join(overview.split()), {'style': 'padding-left: 20'}) + 'Column legend<br />' + html_tag('ul', html)
...
def do_html_contam_art_first_n_pb_tr(stats, files, odir, filepath_prefix):
    ...
    temp = os.path.join(exedir, 'template/readqc_artifacts.html')
    html = ''
...
def do_html_body(odir, filepath_prefix):
    exedir = os.path.dirname(os.path.abspath(__file__))
    temp = os.path.join(exedir, 'template/readqc_body_template.html')
    ...
    tok_map = {
            ...
            'overall bases Q score mean' : {'token' : '[_BASE-QUALITY-SCORE_]', 'type': 'raw'},
            'overall bases Q score std' : {'token' : '[_BASE-QUALITY-SCORE-STD_]', 'type': 'raw'},
            'Q30 bases Q score mean' : {'token' : '[_Q30-BASE-QUALITY-SCORE_]', 'type': 'raw'},
            'Q30 bases Q score std' : {'token' : '[_Q30-BASE-QUALITY-SCORE-STD_]', 'type': 'raw'},
            ...
            'SUBSAMPLE_RATE' : {'token' : '[_SUBSAMPLE-RATE_]', 'type': 'pct'},
            ...
            ## Insert Size
            'ILLUMINA_READ_INSERT_SIZE_JOINED_PERC' : {'token' : '[_PCT-READS-JOINED_]', 'type': 'raw'},
            ...
            ## Read GC
            'read GC mean' : {'token' : '[_READ-GC-AVG_]', 'type': 'raw'},
            'read GC std' : {'token' : '[_READ-GC-STDDEV_]', 'type': 'raw'},
            ...
            'illumina read percent contamination artifact seal' : {'token' : '[_CONTAM-ART-SEA-PCT_]', 'type': 'float'},
            'artifact.seal.stats seal' : {'token' : '[_CONTAM-ART-SEAL_]', 'type': 'file'},

            'illumina read percent contamination DNA spikein seal' : {'token' : '[_DNA-SPIKEIN-SEAL_PCT_]', 'type': 'float'},
            'DNA_spikein.seal.stats seal' : {'token' : '[_DNA-SPIKEIN-SEAL_]', 'type': 'file'},

            'illumina read percent contamination RNA spikein seal' : {'token' : '[_RNA-SPIKEIN-SEAL_PCT_]', 'type': 'float'},
            'RNA_spikein.seal.stats seal' : {'token' : '[_RNA-SPIKEIN-SEAL_]', 'type': 'file'},

            'illumina read percent contamination fosmid seal' : {'token' : '[_CONTAM-FOSMID-SEAL-PCT_]', 'type': 'float'},
            'fosmid.seal.stats seal' : {'token' : '[_CONTAM-FOSMID-SEAL_]', 'type': 'file'},

            'illumina read percent contamination fosmid seal' : {'token' : '[_CONTAM-FOSMID-SEAL-PCT_]', 'type': 'float'},
            'fosmid.seal.stats seal' : {'token' : '[_CONTAM-FOSMID-SEAL_]', 'type': 'file'},

            'illumina read percent contamination mitochondrion seal' : {'token' : '[_CONTAM-MITO-SEAL-PCT_]', 'type': 'float'},
            'mitochondrion.seal.stats seal' : {'token' : '[_CONTAM-MITO-SEAL_]', 'type': 'file'},

            'illumina read percent contamination plastid seal' : {'token' : '[_CONTAM-CHLO-SEAL-PCT_]', 'type': 'float'},
            'plastid.seal.stats seal' : {'token' : '[_CONTAM-CHLO-SEAL_]', 'type': 'file'},

            'illumina read percent contamination phix seal' : {'token' : '[_CONTAM-PHIX-SEAL-PCT_]', 'type': 'float'},
            'phix.seal.stats seal' : {'token' : '[_CONTAM-PHIX-SEAL_]', 'type': 'file'},

            'illumina read percent contamination rrna seal' : {'token' : '[_CONTAM-RRNA-SEAL-PCT_]', 'type': 'float'},
            'rrna.seal.stats seal' : {'token' : '[_CONTAM-RRNA-SEAL_]', 'type': 'file'},

            'illumina read percent contamination microbes seal' : {'token' : '[_CONTAM-NON-SYN-SEAL-PCT_]', 'type': 'float'},
            'microbes.seal.stats seal' : {'token' : '[_CONTAM-NON-SYN-SEAL_]', 'type': 'file'},

            # 'illumina read percent contamination synthetic seal' : {'token' : '[_CONTAM-SYN-SEAL-PCT_]', 'type': 'float'},
            'synthetic.seal.stats seal' : {'token' : '[_CONTAM-SYN-SEAL_]', 'type': 'file'},

            'illumina read percent contamination adapters seal' : {'token' : '[_CONTAM-ADAPTER-PCT_]', 'type': 'float'},
    ...
    with open(temp, 'r') as fh:
        ...
        for key in tok_map:
            dat = tok_map[key]
            html = html.replace(dat['token'], pipeline_val(key, dat, stats, files, filepath_prefix))
        ...
        if os.path.isfile(sketch_nt_file):
            ...
            sketch_html +=  sketch_table(sketch_nt_file)
...
def do_html(odir, infile):
    ...
    temp = os.path.join(exedir, 'template/readqc_template.html')
