...
"""
 To set tool path.
 Try "module load" first and do "which".
 If no module supported, just try "which".

 Note: This is not actually load the modules. You should load those modules for the production runs.

"""
def get_tool_path(binName, moduleName=None, log=None):
    stdOut = None

    ## 05012017 To support Denovo/Cori
    if os.environ['NERSC_HOST'] in ("denovo", "cori"):
        if moduleName == "bbtools":
            # stdOut = "shifter --image=sulsj/bbtools:37.48 /bbmap/%s" % (binName) ## with rqcfilter.sh
            stdOut = "shifter --image=bryce911/bbtools /bbmap/%s" % (binName) ## with rqcfilter.sh
        elif moduleName in ("blast+", "blast+/2.6.0", "blast+/2.7.0"):
            stdOut = "shifter --image=sulsj/ncbi-blastplus:2.6.0 %s" % (binName) ## revert back to 2.6 01292018. Requested by jasmyn and Alex S
            # stdOut = "shifter --image=sulsj/ncbi-blastplus:2.7.0 %s" % (binName) ## 10132017
        elif moduleName == "timeout":
            stdOut = "timeout"
        elif moduleName == "gnuplot":
            stdOut = "shifter --image=bryce911/bbtools %s" % (binName)
        elif moduleName == "velvet":
            stdOut = "shifter --image=bioboxes/velvet:latest %s" % (binName)
        elif moduleName == "R":
            stdOut = "Rscript"
        elif moduleName == "last":
            stdOut = "shifter --image=bryce911/lastal:852 %s" % (binName)
        elif moduleName == "fastqc":
            stdOut = "shifter --image=sulsj/fastqc:0.11.5 %s" % (binName)
        elif moduleName == "diamond":
            stdOut = "shifter --image=sulsj/diamond:0.9.10 /%s" % (binName)

    if stdOut is None: ## if not on Denovo/Cori or need other tools
        ## exitCode from module load is not reliable.
        ## Thus,
        ## if stdErr has ERROR ==> no module support
        ## elif stdOut is None ==> module exists but the bin doesn't
        ## else OK!
        ##
        stdOut, stdErr, exitCode = run_sh_command("module load %s && which %s" % (moduleName, binName), True, log)
        #assert exitCode == 0

        if os.environ['NERSC_HOST'] == "genepool" and moduleName == "diamond":
            ## genepool module's diamond is 0.7.1 version.
            ## The latest diamond can be found from /global/dna/projectdirs/PI/rqc/prod/tools
            stdErr = "ERROR"
        ...
        if stdErr.find("ERROR") != -1 or stdErr.find("command not found") != -1: ## module loading fails
            ...
            paths = paths + ":" + os.path.join(get_run_path(), "pacbio/tools")
            paths = paths + ":/global/dna/projectdirs/PI/rqc/prod/tools"

            ## TODO: shell dependency. Might need to get the shell name first. Assuming bash for now
            stdOut, stdErr, exitCode = run_sh_command("export PATH=%s:$PATH && which %s" % (paths, binName), True, log)
            # assert exitCode == 0

    ## genepool's timeout needs flags
    if os.environ['NERSC_HOST'] == "genepool":
        if stdOut.rstrip().endswith("timeout"):
            stdOut = stdOut.rstrip() + " -v -t"

    ## only for jgi=rqc docker
    if not stdOut:
        stdOut, stdErr, exitCode = run_sh_command("which %s" % (binName), True, log)
        if not stdOut: stdOut, stdErr, exitCode = run_sh_command("export PATH=/bbmap:$PATH && which %s" % (binName), True, log)


    ## Still nothing in stdOut?
    if not stdOut:
        return "Failed to find '%s' installation.\n" % (binName)

    else:
        if stdOut.find("/jgi/tools/") != -1:
            if log:
                log.error("Still accessing '/jgi/tools' for %s! Please remove '/jgi/tools/' from PATH environment variable." % (binName))
            else:
                print >>sys.stderr, "Still accessing '/jgi/tools' for %s! Please remove '/jgi/tools/' from PATH environment variable." % (binName)

            sys.exit(1)

        if stdOut.find("/house/") != -1:
            if log:
                log.error("Still accessing '/house' for %s!" % (binName))
            else:
                print >>sys.stderr, "Still accessing '/house/' for %s!" % (binName)

            sys.exit(1)

        if log:
            log.info("Command found: %s" % (stdOut.rstrip()))
...
if __name__ == "__main__":
    MEGABLAST_CMD = get_tool_path("megablast", "blast")
    print MEGABLAST_CMD
    FORMATDB_CMD = get_tool_path("formatdb", "blast")
    print FORMATDB_CMD
    BLASTALL_CMD = get_tool_path("blastallxxx", "blast")
    print BLASTALL_CMD
    BLASTALL_CMD = get_tool_path("blastall", "blast")
    print BLASTALL_CMD
    print get_tool_path("blastn", "blast+")

    print get_tool_path("stats.sh", "bbtools")

    AGREP_CMD = get_tool_path("agrep", "agrep")
    print AGREP_CMD
    TAGDUST_CMD = get_tool_path("tagdust", "tagdust")
    print TAGDUST_CMD

    print get_tool_path("gnuplot", "gnuplot")
    print get_tool_path("bwa", "bwa")

    print get_tool_path("cat", "cat")
    print get_tool_path("bzcat", "bzcat")
    print get_tool_path("zcat", "zcat")
    print get_tool_path("head", "head")
    print get_tool_path("tail", "tail")
    print get_tool_path("perl", "perl")
    print get_tool_path("grep", "grep")
    print get_tool_path("nawk", "nawk")

    print get_tool_path("xxx", "yyy")
    print get_tool_path("rm", "rm") + " -f"
    print get_tool_path("mkdir", "mkdir") + " -p"
    print get_tool_path("cplusmersampler", "cplusmersampler")
