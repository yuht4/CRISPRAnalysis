...
from readqc_constants import RQCReadQcConfig, RQCReadQcCommands, RQCContamDb, RQCReadQcReferenceDatabases, RQCReadQc
...
def fast_subsample_fastq_sequences(sourceFastq, outputFileName, subsamplingRate, isStatGenOnly, log):
    ## Tools
    bbtoolsReformatShCmd = RQCReadQcCommands.BBTOOLS_REFORMAT_CMD
    ...
    reformatPrefix = subsampledFile.replace(".fastq", "")
...
def write_unique_20_mers(fastq, log):
    ## Tools
    bbcountuniqueShCmd = RQCReadQcCommands.BBCOUNTUNIQUE_SH_CMD
...
def illumina_find_common_motifs(fastq, log):
    ## Tools
    patterNFastqPlCmd = os.path.join(os.path.dirname(__file__), 'patterN_fastq.pl') #RQCReadQcCommands.PATTERN_FASTQ_PL
...
def illumina_run_dedupe(fastq, log):
    ## Tools
    bbdedupeShCmd = RQCReadQcCommands.BBDEDUPE_SH
...
def illumina_sciclone_analysis2(origFastq, isPairedEnd, log, libName=None, isRna=None):
    ...
    bbdukShCmd = RQCReadQcCommands.BBDUK_SH_CMD
...
def end_of_read_illumina_adapter_check(firstSubsampledFastqFile, log):
    kmercountPosCmd = os.path.join(os.path.dirname(__file__), 'kmercount_pos.py')    #RQCReadQcCommands.KMERCOUNT_POS_CMD
...
def insert_size_analysis(fastq, log):
    ## Tools
    bbmergeShCmd = RQCReadQcCommands.BBMERGE_SH_CMD
...
def run_blastplus_py(queryFastaFile, db, log):
    timeoutCmd = RQCReadQcCommands.TIMEOUT_CMD
