...
# class ReadQCPaths:
#     RQC_SW = get_run_path()
#     RQC_SW_TOOL_DIR = os.path.join(RQC_SW, "readqc/tools")


class RQCReadQcCommands:
    ## STEP1
    BBTOOLS_REFORMAT_CMD = get_tool_path("reformat.sh", "bbtools")

    ## STEP2
    BBCOUNTUNIQUE_SH_CMD = get_tool_path("bbcountunique.sh", "bbtools")
    SHUFFLE_SH_CMD = get_tool_path("shuffle.sh", "bbtools")

    ## STEP8
    PATTERN_FASTQ_PL = get_tool_path("patterN_fastq.pl", "patterN_fastq.pl")

    ## STEP9
    BBDEDUPE_SH = get_tool_path("dedupe.sh", "bbtools")

    ## STEP5 & STEP11 ==> replaced by bbduk 03032015
    SEAL_SH_CMD = get_tool_path("seal.sh", "bbtools")

    ## STEP12
    BBDUK_SH_CMD = get_tool_path("bbduk.sh", "bbtools")

    ## STEP13,14,15
    BLASTN_CMD = get_tool_path("blastn", "blast+")
    TIMEOUT_CMD = get_tool_path("timeout", "timeout")
    ...
    ## STEP18
    BBMERGE_SH_CMD = get_tool_path("bbmerge.sh", "bbtools")

    ## STEP19
    GC_DIVERGENCE_TRANSFORM = get_tool_path("gc_divergence_transform.r", "gc_divergence_transform.r")
    GC_DIVERGENCE_MODEL = get_tool_path("gc_divergence_model.r", "gc_divergence_model.r")
    GC_DIVERGENCE_PLOT = get_tool_path("gc_divergence_plot.r", "gc_divergence_plot.r")

    ## STEP20
    # RSCRIPT_CMD = get_tool_path("Rscript", "R/3.3.1") ## genepool's lastest R version
