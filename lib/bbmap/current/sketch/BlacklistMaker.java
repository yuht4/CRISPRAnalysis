	...
	public BlacklistMaker(String[] args){
		...
		for(int i=0; i<args.length; i++){
			...
			if(a.equals("verbose")){
				...
				minTaxCount=(int)Tools.parseKMG(b);
...
//			System.err.println("prebits="+prebits+", currentPass="+currentPass);
...

		{
			...
			filter[0]=KmerCount7MTA.makeKca(in1, in2, extra, k, cbits, 0, precells, prehashes, minq, rcomp, false, maxReads, 1, 1, 1, 1, filter[0], filterMax, amino);
	...
	private class ProcessThread extends Thread {
		...
		ProcessThread(final ConcurrentReadInputStream cris_, final int tid_){
			...
			if(defaultParams.minEntropy>0){
				eTracker=new EntropyTracker(entropyK, k, defaultParams.minEntropy, true);
			}else{
				eTracker=null;
			}
		}
