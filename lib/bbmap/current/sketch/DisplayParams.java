...
//			assert(minSizeRatio>=0f && minSizeRatio<=1.0f) : "\nminSizeRatio must be between 0 and 1, inclusive.\n";
		...
		}else if(a.equalsIgnoreCase("anifromwkid")){
			aniFromWkid=Tools.parseBoolean(b);
		}
...
//		}
	...
	public String toString(){
		...
		if(!aniFromWkid){sb.append(" aniFromWkid=").append(aniFromWkid);}
...
//		printColors=true;
	...
	public JsonObject toJson(Comparison c){
		...
		j.add("WKID", 100*c.idMinDivisor());
		j.add("KID", 100*c.idMaxDivisor());
		
		//Primary fields
		if(printAni){j.add("ANI", 100*c.ani(aniFromWkid));}
...
//		sb.append(String.format(Locale.ROOT, "%.2f%%\t%.2f%%", 100*c.idMinDivisor(), 100*c.idMaxDivisor()));
		sb.append(100*c.idMinDivisor(), 2).append('%').append('\t');
		sb.append(100*c.idMaxDivisor(), 2).append('%');

//		if(printAni){sb.append(String.format(Locale.ROOT, "\t%.2f%%", 100*c.ani(aniFromWkid)));}
...
//		if(printUContam){sb.append(String.format(Locale.ROOT, "\t%.2f%%", 100*c.uContamFraction()));}
		...
		if(printAni){sb.append('\t').append(100*c.ani(aniFromWkid), 2).append('%');}
	...
	void formatComparison3Column(Comparison c, ByteBuilder sb, int prevTid, Sketch query){
...
//		sb.append(rName).append(String.format(Locale.ROOT, "\t%.2f\t%d\t%d\t%d", 100*c.ani(aniFromWkid), sea, seb, ba));
	...
	void formatComparison(Comparison c, ByteBuilder sb, int prevTaxID, Sketch query){
		if(format==FORMAT_MULTICOLUMN){
			formatComparisonColumnwise(c, sb, prevTaxID);
			return;
		}else if(format==FORMAT_QUERY_REF_ANI){
...
//		String score=(printScore ? String.format(Locale.ROOT, "\tscore %.2f", c.score2()) : "");
	...
	public boolean aniFromWkid=true;
...
//	IntList bannedTaxid=null;
	...
	public static final int FORMAT_OLD=0, FORMAT_MULTICOLUMN=2, FORMAT_QUERY_REF_ANI=3, FORMAT_JSON=4;
