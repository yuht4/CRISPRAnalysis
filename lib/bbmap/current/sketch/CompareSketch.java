...
//		assert(Primes.primes.length>0);
//		System.err.println(Primes.primes.length);
//		
//		for(int i=0; i<100; i++){
//			Primes.fetchPrimes();
//		}
	...
	public CompareSketch(String[] args){
		...
		if(searcher.refFiles.isEmpty()){throw new RuntimeException("Error - at least one reference file is required.");}
...
//		assert(defaultParams.checkValid());
	...
	public void process(Timer t){
		...
		if(tsw!=null){
			tsw.start();
			if(defaultParams.format==DisplayParams.FORMAT_QUERY_REF_ANI){tsw.println(defaultParams.header());}
		}
