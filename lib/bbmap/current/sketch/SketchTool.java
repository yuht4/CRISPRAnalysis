...
//	}
	...
	public ArrayList<Sketch> loadSketches_MT(int mode, float samplerate, long reads, float minEntropy, String...fnames){
		...
		if(decomposedFnames.size()==1){return loadSketchesFromFile(decomposedFnames.poll(), null, mode, 0, samplerate, reads, minEntropy, false);}
	...
	public ArrayList<Sketch> loadSketchesFromFile(final String fname0, SketchMakerMini smm, 
		...
		if(ff.isSequence()){
			...
			return loadSketchesFromSketchFile(ff, allowZeroSizeSketch);
		}
	}
...
//				allowMultithreadedFastq+", "+forceDisableMultithreadedFastq+", "+(mode==ONE_SKETCH);
		...
		if(ff.fastq() && allowMultithreadedFastq && !forceDisableMultithreadedFastq && mode==ONE_SKETCH &&
	...
	private ArrayList<Sketch> loadSketchesFromSketchFile(final FileFormat ff, boolean allowZeroSizeSketch){
		...
		ArrayList<Sketch> sketches=new ArrayList<Sketch>();
//		ByteFile1 bf=(ByteFile1) ByteFile.makeByteFile(ff); //was (fname, false);
//		ByteFile2 bf=(ByteFile2) ByteFile.makeByteFile(ff); //was (fname, false);
		ByteFile bf=ByteFile.makeByteFile(ff); //was (fname, false);
...
//					if(line.length>1){//Should be faster, but is slower, possibly because more stuff gets JIT'd
//						ArrayList<byte[]> split=Tools.split(line, 1, (byte)'\t');
//						for(byte[] s : split){
//							final int colon=Tools.indexOf(s, (byte)':');
//							final int a=colon+1;
////							final String sub=s.substring(colon+1);
//							if(Tools.startsWith(s, "SZ:") || Tools.startsWith(s, "SIZE:")){//Sketch length
//								currentSketchSize=Tools.parseInt(s, a, s.length);
//							}else if(Tools.startsWith(s, "CD:")){//Coding
//								A48=HEX=NUC=delta=counts=unsorted=false;
//								
...
//									else{assert(false) : "Unknown coding symbol: "+c+"\t"+new String(line);}
//								}
//							}else if(Tools.startsWith(s, "K:")){//Kmer length
//								final int comma=Tools.indexOf(s, (byte)',');
//								if(comma>=0){
//									int x=Tools.parseInt(s, a, comma);
//									int y=Tools.parseInt(s, comma+1, s.length);
//									k_sketch=Tools.max(x, y);
//									k2_sketch=Tools.min(x, y);
//								}else{
//									k_sketch=Tools.parseInt(s, a, s.length);
//									k2_sketch=0;
//								}
//							}else if(Tools.startsWith(s, "H:")){//Hash version
//								int hashVersion_sketch=Tools.parseInt(s, a, s.length);
//								assert(hashVersion_sketch==HASH_VERSION) :
//									"Sketch hash_version "+hashVersion_sketch+" differs from loaded hash_version "+HASH_VERSION+"\n"+new String(line);
//							}else if(Tools.startsWith(s, "GS:") || Tools.startsWith(s, "GSIZE:")){//Genomic bases
//								genomeSizeBases=Tools.parseLong(s, a, s.length);
//							}else if(Tools.startsWith(s, "GK:") || Tools.startsWith(s, "GKMERS:")){//Genomic kmers
//								genomeSizeKmers=Tools.parseLong(s, a, s.length);
//							}else if(Tools.startsWith(s, "GQ:")){
//								genomeSequences=Tools.parseLong(s, a, s.length);
//							}else if(Tools.startsWith(s, "GE:")){//Genome size estimate kmers
//								//ignore
//							}else if(Tools.startsWith(s, "PC:")){//Probability of correctness
//								probCorrect=Tools.parseFloat(s, a, s.length);
//							}else if(Tools.startsWith(s, "ID:") || Tools.startsWith(s, "TAXID:")){
//								taxID=Tools.parseInt(s, a, s.length);
//							}else if(Tools.startsWith(s, "IMG:")){
//								imgID=Tools.parseLong(s, a, s.length);
//							}else if(Tools.startsWith(s, "SPID:")){
//								spid=Tools.parseInt(s, a, s.length);
//							}else if(Tools.startsWith(s, "NM:") || Tools.startsWith(s, "NAME:")){
//								name=new String(s, a, s.length-a);
//							}else if(Tools.startsWith(s, "FN:")){
//								fname=new String(s, a, s.length-a);
//							}else if(Tools.startsWith(s, "NM0:")){
//								name0=new String(s, a, s.length-a);
//							}else if(Tools.startsWith(s, "MT_")){
//								if(meta==null){meta=new ArrayList<String>(1);}
//								meta.add(new String(s, 3, s.length-3));
//							}else{
//								assert(false) : "Unsupported header tag "+s;
//							}
//						}
	...
	private class LoadThread extends Thread{
		...
		public void run(){
			...
			for(String fname=queue.poll(); fname!=null; fname=queue.poll()){
				...
				try {
					temp=loadSketchesFromFile(fname, smm, smm.mode, 1, samplerate, reads, smm.minEntropy(), false);
				} catch (Throwable e) {
