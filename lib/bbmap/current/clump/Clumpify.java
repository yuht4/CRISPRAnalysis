...
//		String temp2=temp.replace("%", "FINAL");
		...
		if(externalSort){
			...
			SortByName.mergeAndDump(names, null, dest, dest2, delete, useSharedHeader);
		}
...
//		outstream.println("Raw Disk Size Estimate: "+(long)(diskEstimate/(1024*1024))+" MB");
		...
		if(maxMem>worstCase && readEstimate<Integer.MAX_VALUE){
			...
			groups=Tools.max(11, (3+(int)((3*worstCase/maxMem)*(V3 ? KmerSort3.fetchThreads : 2)))|1, (int)((2*readEstimate)/Integer.MAX_VALUE))|1;
		}
