...
//			}
	...
	public final void process(Timer t){
		...
		if(showSpeed){
			...
			if(processingMode==extendMode || processingMode==correctMode || processingMode==discardMode){
				outstream.println("\nReads Processed:    "+rpstring+" \t"+String.format(Locale.ROOT, "%.2fk reads/sec", rpnano*1000000));
...
//		outstream.println("Load Time:                  \t"+t);
		...
		if(mode==extendMode || mode==correctMode || mode==discardMode){
			...
			if(ecc){
				...
				outstream.println("Reads with errors detected: \t"+readsDetected+String.format(Locale.ROOT, " \t(%.2f%%)", readsDetected*100.0/readsIn));
				outstream.println("Reads fully corrected:      \t"+readsFullyCorrected+String.format(Locale.ROOT, " \t(%.2f%% of detected)", readsFullyCorrected*100.0/readsDetected));
				outstream.println("Reads partly corrected:     \t"+partial+String.format(Locale.ROOT, " \t(%.2f%% of detected)", partial*100.0/readsDetected));
				if(ECC_ROLLBACK || rollbacks>0){
					outstream.println("Rollbacks:                  \t"+rollbacks+String.format(Locale.ROOT, " \t(%.2f%% of detected)", rollbacks*100.0/readsDetected));
				}
