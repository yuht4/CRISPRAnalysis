...
//			System.err.println("prebits="+prebits+", currentPass="+currentPass);
		...
		if(onePass){
			...
			filter[0]=KmerCount7MTA.makeKca(null, null, null, kbig(), cbits, 0, precells, prehashes, minq, true, ecco(),
					maxReads, 1, 1, 1, 1, null, 0, Shared.AMINO_IN);
		}else{
			if(ht==null){ht=new Timer();}
			ht.start();
			
			filter[0]=KmerCount7MTA.makeKca_als(in1, in2, extra, kbig(), cbits, 0, precells, prehashes, minq, true, ecco(),
