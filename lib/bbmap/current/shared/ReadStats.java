...
//		}
	...
	private static int writeBaseContentToFile2(TextStreamWriter tsw, LongList[] lists, int offset){
		...
		for(int i=0; i<max; i++){
			...
			sb.append(String.format(Locale.ROOT, "%.5f\t", a*mult));
			sb.append(String.format(Locale.ROOT, "%.5f\t", c*mult));
			sb.append(String.format(Locale.ROOT, "%.5f\t", g*mult));
			sb.append(String.format(Locale.ROOT, "%.5f\t", t*mult));
			sb.append(String.format(Locale.ROOT, "%.5f\n", n*mult));
