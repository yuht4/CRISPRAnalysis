...
//			outstream=pp.outstream;
		...
		try {
			counts=count(fname1, fname2, k, cbits, gap, true, false, false, counts);
		} catch (Exception e) {
...
//		System.out.println(Arrays.toString(freq)+"\n");
	...
	public static KCountArray makeKca(String fname1, String fname2, Iterable<String> extraFiles, int k, int cbits, boolean rcomp, boolean eccByOverlap, boolean amino){
		return makeKca(fname1, fname2, extraFiles, k, cbits, 0, Tools.min(2*k, 35), 1, minQuality, rcomp, eccByOverlap, maxReads, 1, 1, 1, 2, amino);
	}
	
	public static KCountArray makeKca(String fname1, String fname2, Iterable<String> extraFiles,
			int k, int cbits, int gap, int matrixbits, int hashes, int minqual, boolean rcomp, boolean eccByOverlap, long maxreads, boolean amino){
		assert(matrixbits<63);
		return makeKca(fname1, fname2, extraFiles, k, cbits, gap, matrixbits, hashes, minqual, rcomp, eccByOverlap, maxreads, 1, 1, 1, 2, amino);
	}
	
	public static KCountArray makeKca(String fname1, String fname2, Iterable<String> extraFiles,
			int k, int cbits, int gap, int matrixbits, int hashes, int minqual, boolean rcomp, boolean eccByOverlap,
			long maxreads, int passes, int stepsize, int thresh1, int thresh2, boolean amino){
		assert(matrixbits<63);
		return makeKca(fname1, fname2, extraFiles,
				k, cbits, gap, 1L<<matrixbits, hashes, minqual, rcomp, eccByOverlap, maxreads, passes, stepsize, thresh1, thresh2, null, 0, amino);
	}
	
	public static KCountArray makeKca(String fname1, String fname2, Iterable<String> extraFiles,
			int k, int cbits, int gap, long cells, int hashes, int minqual, boolean rcomp, boolean eccByOverlap,
			long maxreads, int passes, int stepsize, int thresh1, int thresh2, boolean amino){
		return makeKca(fname1, fname2, extraFiles,
				k, cbits, gap, cells, hashes, minqual, rcomp, eccByOverlap, maxreads, passes, stepsize, thresh1, thresh2, null, 0, amino);
	}
	...
	public static KCountArray makeKca_als(ArrayList<String> fname1, ArrayList<String> fname2, Iterable<String> extraFiles,
		...
		return makeKca(a, b, list.isEmpty() ? null : list, k, cbits, gap, cells, hashes, minqual, rcomp, eccByOverlap,
				maxreads, passes, stepsize, thresh1, thresh2, prefilter, prefilterLimit_, amino_);
	}
...
//			System.out.println("c1");
			try {
				count(fname1, fname2, k, cbits, gap, rcomp, eccByOverlap, amino, kca);
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			if(extraFiles!=null){
				...
				for(String s : extraFiles){
					try {
						count(s, null, k, cbits, gap, rcomp, eccByOverlap, amino, kca);
					} catch (Exception e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
...
//			System.out.println("c2");
			...
			for(int i=1; i<passes; i++){
				...
				try {
					count(fname1, fname2, k, cbits, rcomp, eccByOverlap, kca, trusted, maxreads, thresh1, step, conservative, amino);
				} catch (Exception e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
				if(extraFiles!=null){
					...
					for(String s : extraFiles){
						try {
							count(s, null, k, cbits, rcomp, eccByOverlap, kca, trusted, maxreads, thresh1, step, conservative, amino);
						} catch (Exception e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
						}
			...
			try {
				count(fname1, fname2, k, cbits, rcomp, eccByOverlap, kca, trusted, maxreads, thresh2, stepsize, true, amino);
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			if(extraFiles!=null){
				...
				for(String s : extraFiles){
					try {
						count(s, null, k, cbits, rcomp, eccByOverlap, kca, trusted, maxreads, thresh2, stepsize, true, amino);
					} catch (Exception e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
...
//		System.out.println("d");
	...
	public static KCountArray count(String reads1, String reads2, int k, int cbits, int gap,
			boolean rcomp, boolean eccByOverlap, boolean amino, KCountArray counts) throws Exception{
...
//		assert(false) : THREADS;
		...
		for(int i=0; i<cta.length; i++){
			cta[i]=new CountThread(cris, k, rcomp, eccByOverlap, amino, counts);
			cta[i].start();
		}
...
//		assert(false) : THREADS;
		CountThread[] cta=new CountThread[THREADS];
		for(int i=0; i<cta.length; i++){
			cta[i]=new CountThread(cris, k, rcomp, eccByOverlap, amino, counts, trusted, thresh, detectStepsize, conservative);
			cta[i].start();
		}
...
//		System.out.println("\ncount=\n"+count);
	...
	private static class CountThread extends Thread{
		...
		CountThread(final ConcurrentReadInputStream cris_, final int k_, final boolean rcomp_, final boolean eccByOverlap_, final boolean amino_,
				final KCountArray counts_){
			this(cris_, k_, rcomp_, eccByOverlap_, amino_, counts_, null, 2, 1, true);
		}
...
//			System.out.println("Finished: "+readsProcessedLocal);
		...
		private int increment(final long key, final KCountArray counts){
			if(SKETCH_MODE){//Stores kmers using k, ignoring k2.
				final long code=SketchTool.hash(key);
