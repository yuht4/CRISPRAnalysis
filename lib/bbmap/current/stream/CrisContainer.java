...
public class CrisContainer implements Comparable<CrisContainer> {
	...
	public CrisContainer(String fname, int count_, Comparator<Read> comparator_){
	...
	public CrisContainer(ConcurrentReadInputStream cris_, int count_, Comparator<Read> comparator_){
		comparator=comparator_;
		remainingReads=count=count_;
		randy=(count>0) ? new Random() : null;
		cris=cris_;
