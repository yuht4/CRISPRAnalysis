	...
	public static byte testQuality(ArrayList<String> oct){
		...
		for(int k=0; k<2; k++){
			...
			for(int i=0; i<quals.length; i++){
					...
					if(ASCII_OFFSET==33 && (q>QUAL_THRESH || (bases[i]=='N' && (q==31 || q==33)))){
						if(warnQualityChange && qflips<4){
							System.err.println("Changed from ASCII-33 to ASCII-64 on input quality "+(char)q0+" (Q"
									+(q+ASCII_OFFSET)+") for base "+(char)(bases[i])+" at lines "+a+" and "+b+", position "+i+" while prescanning."
				...
				assert(q>=-5 || IGNORE_BAD_QUALITY) : "ASCII encoding for quality (currently ASCII-"+ASCII_OFFSET+") appears to be wrong \nfor input quality "
								+(q+ASCII_OFFSET)+" for base "+(char)(bases[i])+" at lines "+a+" and "+b+", position "+i+".\n"
...
//		int longest=0;
		...
		for(s=tf.nextLine(); s!=null && added<maxReadsToReturn; s=tf.nextLine()){
			...
			if(cntr==4){
				...
				Read r=null;
//				r=quadToRead_fast(quad, tf, numericID);
//				if(!FAST_FAILED){r=quadToRead_fast(quad, tf, numericID);}
//				if(r==null){r=quadToRead_slow(quad, false, tf, numericID);}
				r=quadToRead_slow(quad, false, tf, numericID);
...
//				assert(false);
	...
	public static Read quadToRead_slow(final byte[][] quad, boolean scarf, ByteFile tf, long numericID){
...
//			assert(false);
	...
	public static Read quadToRead_fast(final byte[][] quad, ByteFile tf, long numericID){
		...
		if(header==null || header.length<1 || header[0]!=(byte)'@'){return quadToRead_slow(quad, false, tf, numericID);}
...
//			over|=quals[i]>QUAL_THRESH;
		...
		if(bad/*over || negative<0*/){
			...
			return quadToRead_slow(quad, false, tf, numericID);
		}
	...
	public static ArrayList<Read> toScarfReadList(ByteFile tf, int maxReadsToReturn, long numericID, boolean interleaved){
		...
		for(s=tf.nextLine(); s!=null && added<maxReadsToReturn; s=tf.nextLine()){
			scarfToQuad(s, quad);
			Read r=quadToRead_slow(quad, true, tf, numericID);
