...
public class TaxTree implements Serializable{
	...
	private void assignStrains(){
		...
		System.out.println("Assigning strains.");
		int strains=0, substrains=0;
		TaxNode bacteria=getNode(2); //Can't do a name lookup since the names are not hashed
		assert(bacteria.name.equalsIgnoreCase("Bacteria"));
		
		ArrayList<TaxNode> bactList=new ArrayList<TaxNode>();
		for(TaxNode tn : nodes){
			if(tn!=null && tn.originalLevel()==NO_RANK && tn.minParentLevelExtended<=SPECIES_E && commonAncestor(tn, bacteria)==bacteria){
				bactList.add(tn);
			}
		}
		
		for(TaxNode tn : bactList){
...
//		System.out.println("Assigned "+strains+" strains.");
		for(TaxNode tn : bactList){
			if(tn.maxDescendantLevelIncludingSelf()==NO_RANK){
				TaxNode parent=nodes[tn.pid];
...
//		System.out.println("Assigned "+substrains+" substrains.");
	}
...
//		System.out.println("Assigned "+substrains+" substrains.");
	...
	private TaxTree(String namesFile, String nodesFile, String mergedFile){
		...
		countChildren();
		System.out.println("Counted children.");
		int rounds=percolate();
		System.out.println("Percolated "+rounds+" rounds to fixpoint.");
...
//		}
	...
	public synchronized void hashNames(boolean genusDotSpecies){
		...
		final int size=((int)Tools.mid(2, (nodes.length+(genusDotSpecies ? nodesPerLevelExtended[SPECIES_E] : 0))*1.5, Integer.MAX_VALUE));
	...
	public synchronized void hashChildren(){
		...
		childMap=new HashMap<TaxNode, ArrayList<TaxNode>>((int)Tools.mid(2, nodesWithChildren*1.5, Integer.MAX_VALUE));
		for(TaxNode tn : nodes){
			if(tn!=null){
				if(tn.numChildren>0){
