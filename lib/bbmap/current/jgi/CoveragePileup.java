	...
	public boolean addCoverage(final String scafName, final byte[] seq, byte[] match, final int start0, final int stop0, final int readlen, final int strand, int incrementFrags,
		...
		if(scaf==null){
			if(ADD_FROM_READS){
				...
				list.add(scaf);
				return addCoverage(scaf, seq, match, start0, stop0, readlen, strand, incrementFrags, properPair);
			}else if(EA){
	...
	public boolean processSamLine(SamLine sl){
		...
		if(PHYSICAL_COVERAGE && properPair){
			...
			if(mate==null){pairTable.put(sl.qname, sl);}
				...
				mappedKmers+=kmers;
				addCoverage(sl.rnameS(), null, null, Tools.min(start1, start2), Tools.max(stop1, stop2), length, strand, 2, sl.properPair());
			}
...
//			assert(false) : "'"+sl.rnameS()+"'";
			...
			mappedKmers+=kmers;
			return addCoverage(sl.rnameS(), sl.seq, match, start, stop, length, sl.strand(), sl.hasMate() ? 1 : 2, sl.properPair());
		}
		return false;
	}
		...
		if(r.mapped() && r.bases!=null){
			if(USE_SECONDARY && r.sites!=null && r.sites.size()>0){
				...
				if(set1 && set2 && Tools.equals(coords.name, coords2.name)){
					...
					mappedKmers+=kmers;
					addCoverage(new String(coords.name), null, null, Tools.min(start1, start2), Tools.max(stop1, stop2), length, strand, 2-r.mateCount(), r.paired());
				}else{
					if(set1){
						mappedKmers+=kmers;
						return addCoverage(new String(coords.name), r.bases, r.match, coords.start, coords.stop, r.length(), coords.strand, 2-r.mateCount(), r.paired());
					}
				}
			}
		}
	...
	public boolean processRead(Read r, SiteScore ss){
		if(ss!=null && r.bases!=null){
			if(coords.set(ss)){
				return addCoverage(new String(coords.name), r.bases, ss.match, coords.start, coords.stop, r.length(), coords.strand, 1, r.paired());
			}
		}
		return false;
	}
...
//			outstream.println(kmersProcessed+", "+correctKmers);
		...
		outstream.println(String.format(Locale.ROOT, "Average coverage:                    \t%.3f", depthCovered));
