...
//				assert(false) : b+", "+Arrays.toString(literal);
				...
				if("lc".equalsIgnoreCase(b) || "lowercase".equalsIgnoreCase(b)){
					...
					if(b!=null && b.length()==1 && !b.equalsIgnoreCase("f")){
						ktrimLeft_=false;ktrimRight_=false;ktrimN_=true;
						TRIM_SYMBOL_=(byte)b.charAt(0);
					}else{
						boolean x=Tools.parseBoolean(b);
...
//			requireBothBad=parser.requireBothBad;
		...
		if(ktrimLeft_ || ktrimRight_ || ktrimN_){
			if(kbig_>k_){
				...
				outstream.println("WARNING: When kmer-trimming, the maximum value of K is "+k_+".");
		...
		ffout1=FileFormat.testOutput(out1, FileFormat.FASTQ, null, true, overwrite, append, ordered);
		ffout2=FileFormat.testOutput(out2, FileFormat.FASTQ, null, true, overwrite, append, ordered);
		ffoutb1=FileFormat.testOutput(outb1, FileFormat.FASTQ, null, true, overwrite, append, ordered);
		ffoutb2=FileFormat.testOutput(outb2, FileFormat.FASTQ, null, true, overwrite, append, ordered);
		ffouts=FileFormat.testOutput(outsingle, FileFormat.FASTQ, null, true, overwrite, append, ordered);
			...
			for(int i=k-1; i>=mink; i--){
				...
				if(verbose){
					if((toValue(kmer, rkmer, lengthMasks[i]))%WAYS==tnum){
						outstream.println("added="+x+"; i="+i+"; tnum="+tnum+"; Added left-shift kmer "+AminoAcid.kmerToString(kmer&~lengthMasks[i], i)+"; value="+(toValue(kmer, rkmer, lengthMasks[i]))+"; kmer="+kmer+"; rkmer="+rkmer+"; kmask="+lengthMasks[i]+"; rightMasks[i+1]="+rightMasks[i+1]);
						outstream.println("i="+i+"; tnum="+tnum+"; Looking for left-shift kmer "+AminoAcid.kmerToString(kmer&~lengthMasks[i], i));
...
//				assert(Long.numberOfLeadingZeros(rkmer)>=2*(32-i)) : Long.numberOfLeadingZeros(rkmer)+", "+i+", "+rkmer+", "+kMasks[i];
				...
				if(verbose){
					if((toValue(kmer, rkmer, lengthMasks[i]))%WAYS==tnum){
						outstream.println("added="+x+"; i="+i+"; tnum="+tnum+"; Added right-shift kmer "+AminoAcid.kmerToString(kmer&~lengthMasks[i], i)+"; value="+(toValue(kmer, rkmer, lengthMasks[i]))+"; kmer="+kmer+"; rkmer="+rkmer+"; kmask="+lengthMasks[i]+"; rightMasks[i+1]="+rightMasks[i+1]);
						outstream.println("i="+i+"; tnum="+tnum+"; Looking for right-shift kmer "+AminoAcid.kmerToString(kmer&~lengthMasks[i], i));
						final long value=toValue(kmer, rkmer, lengthMasks[i]);
						if(map.contains(value)){outstream.println("Found "+value);}
					}
				}
					...
					if(!remove){//due to being junk
						...
						if(filterVars){
							if(r1!=null && !r1.discarded()){
								...
								if(!b){r1.setDiscarded(true);}
							}
							if(r2!=null && !r2.discarded()){
								boolean b=passesVariantFilter(r2);
								if(!b){r2.setDiscarded(true);}
							}
						}
						
						if(!r1.discarded() && r1.length()<minlen1){r1.setDiscarded(true);}
						if(r2!=null && !r2.discarded() && r2.length()<minlen2){r2.setDiscarded(true);}
...
//								overlapMargin, overlapMaxMismatches0, overlapMaxMismatches, overlapMinq);
					...
					if(!remove && trimPolyA>0){
						...
						if((removePairsIfEitherBad && (r1.discarded() || (r2!=null && r2.discarded()))) ||
								(r1.discarded() && (r2==null || r2.discarded()))){
							basesPolyTrimmedT+=(r1.length()+r1.mateLength());
							remove=true;
							if(addTrimmedToBad && bad!=null){bad.add(r1);}
						}
					}
					...
					if(!remove && entropyMask){
						...
						if(useEntropyTracker){
							...
							if(r1!=null && !r1.discarded()){
								int masked=maskLowEntropy(r1, null, entropyK, entropyWindowBases,
										entropyCounts, entropyCountCounts, entropyCutoff, entropyKmerspace, verifyEntropy);
								basesEFilteredT+=masked;
								readsEFilteredT+=(masked>0 ? 1 : 0);
							}
							if(r2!=null && !r2.discarded()){
								int masked=maskLowEntropy(r2, null, entropyK, entropyWindowBases,
										entropyCounts, entropyCountCounts, entropyCutoff, entropyKmerspace, verifyEntropy);
								basesEFilteredT+=masked;
								readsEFilteredT+=(masked>0 ? 1 : 0);
							}
					...
					if(!remove){
						...
						if(r1!=null && !r1.discarded()){
							int len=r1.length();
							if(len<minlen1 || len>maxReadLength){r1.setDiscarded(true);}
						}
						if(r2!=null && !r2.discarded()){
							int len=r2.length();
							if(len<minlen2 || len>maxReadLength){r2.setDiscarded(true);}
						}
					...
					if(!remove){
						...
						if(minBaseQuality>0){
							if(r1!=null && r1.quality!=null && r1.minQuality()<minBaseQuality){r1.setDiscarded(true);}
							if(r2!=null && r2.quality!=null && r2.minQuality()<minBaseQuality){r2.setDiscarded(true);}
						}
					...
					if(!remove && calcEntropy && !entropyMask){
						...
						//Old version
//						if(r1!=null && !r1.discarded() && entropyCutoff>averageEntropy(r1.bases, entropyK, entropyWindowBases,
//								entropyCounts, entropyCountCounts, entropyKmerspace, verifyEntropy)){r1.setDiscarded(true);}
//						if(r2!=null && !r2.discarded() && entropyCutoff>averageEntropy(r2.bases, entropyK, entropyWindowBases,
//								entropyCounts, entropyCountCounts, entropyKmerspace, verifyEntropy)){r2.setDiscarded(true);}
						
						if(r1!=null && !r1.discarded() && !eTracker.passes(r1.bases, true)){r1.setDiscarded(true);}
						if(r2!=null && !r2.discarded() && !eTracker.passes(r2.bases, true)){r2.setDiscarded(true);}
...
//			assert(cardinality==r.countNocalls() || y>0) : cardinality+", "+r.countNocalls()+"\n"+r.length()+"\n"+bs+"\n"+r;//123
		...
		/** Copied from BBMask */
		@Deprecated
		private int maskLowEntropy(final Read r, BitSet bs, final int k, final int window,
				final short[] counts, final short[] countCounts, final float cutoff, final int kmerspace, final boolean verify){
...
//					outstream.println("Added "+kmer+"; counts["+kmer+"]="+counts[kmer]);
				}
...
//					outstream.println("Removed "+kmer2+"; count="+counts[kmer2]);
				}
				...
				if(ns<1 && i2>=-1 && i<bases.length && calcEntropy(countCounts, kmerspace)<cutoff){
//					outstream.println("Masked ("+(i2+1)+", "+(i+1)+")");
					bs.set(i2+1, i+1);
				}
			}
		 ...
		 * @param k Kmer length
		 * @param windowBases Window length.
		...
		private float averageEntropy(final byte[] bases, final int k,
				final int windowBases, final short[] counts, final short[] countCounts, final int kmerspace, boolean verify){
			assert(k>0) : "k must be greater than 0";
//			Arrays.fill(counts, 0);

			assert(countCounts[0]==windowBases);
			if(verify){
				for(int c : counts){assert(c==0);}
				for(int i=1; i<countCounts.length; i++){assert(countCounts[i]==0);}
			}
			...
			for(int i=0, i2=-windowBases; i2<bases.length; i++, i2++){
...
//						ns++;
					...
					assert(countCounts[counts[kmer]]>=-1): i+", "+current+", "+Tools.cardinality(counts)+"\n"+Arrays.toString(counts)+"\n"+Arrays.toString(countCounts);
					counts[kmer]++;
					assert(counts[kmer]<=windowBases+1) : Arrays.toString(counts)+"\n"+Arrays.toString(countCounts);
					countCounts[counts[kmer]]++;
					if(verify){
						assert(current==Tools.cardinality(counts)) : current+", "+Tools.cardinality(counts)+"\n"+Arrays.toString(counts);
						assert(Tools.sum(countCounts)>0 && (Tools.sum(countCounts)<=windowBases+1)): current+", "+Tools.cardinality(counts)+"\n"+Arrays.toString(countCounts);
					}
...
//						ns--;
					...
					if(verify){
						assert(current==Tools.cardinality(counts)) : current+", "+Tools.cardinality(counts)+"\n"+Arrays.toString(counts);
						assert(Tools.sum(countCounts)>=0 && (Tools.sum(countCounts)<=windowBases)): current+", "+Tools.cardinality(counts)+"\n"+Arrays.toString(countCounts);
					}
...
//					outstream.println("Removed "+kmer2+"; count="+counts[kmer2]);
				...
				if(verify && i2>-1 && i<bases.length){
					assert(Tools.sum(counts)==windowBases);
					assert(Tools.sum(countCounts)==windowBases): current+", "+Tools.cardinality(counts)+"\n"+Arrays.toString(countCounts);
				}
				
				if(i2>=-1 && i<bases.length){
					 float e=calcEntropy(countCounts, kmerspace);
					 entropySum+=e;
					 entropyMeasurements++;
				}
			}
...
//			outstream.println(" *** ");
//			outstream.println(entropySum+", "+entropyMeasurements+", "+(entropySum/(Tools.max(1, entropyMeasurements))));
//			outstream.println(window+", "+k+", "+kmerspace+", "+counts.length+", "+countCounts.length);
//			outstream.println(" *** ");
		 ...
		 */
		@Deprecated
		private float calcEntropy(short[] countCounts, int kmerspace){
			double eSum=0;
...
//			outstream.println("entropy = "+(sum*entropyMult));
			
			float e=(float)(eSum*entropyMult);
//			outstream.println(String.format("%.3f", eSum)+"\t"+String.format("%.3f", entropyMult)+"\t"+String.format("%.3f", e));
			return (float)e;
		}
	...
	static final long[] lengthMasks;
	
	public static HashMap<String, Long> RQC_MAP=null;
	
	public final boolean useEntropyTracker=true;
