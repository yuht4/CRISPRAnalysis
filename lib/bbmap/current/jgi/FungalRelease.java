...
public class FungalRelease {
	...
	void processInner(final ConcurrentReadInputStream cris, final ConcurrentReadOutputStream ros, final ConcurrentReadOutputStream rosc){
		...
		if(tswl!=null){tswl.start();}
		
		if(ros!=null){
			if(sortScaffolds){Shared.sort(scaffolds, ReadLengthComparator.comparator);}
			if(renameScaffolds){
		...
		if(rosc!=null || makeAgp){//Process contigs
			...
			if(sortContigs){Shared.sort(contigs, ReadLengthComparator.comparator);}
			if(renameContigs){
				for(Read r : contigs){
